 ...
 *
 */
public class MakeMeCryExtension {
	...
	public void diTimDoiUng() {
		...
		for (String key : keySet) {
			...
			copySameData(listNoTmp, listNoKey);
			copySameData(listCoTmp, listCoKey);
			...
			if (Math.round(sum1 * 100) / 100.0 == Math.round(sum2 * 100) / 100.0) {
				...
				listCoAfter.clear();
				listNoAfter.clear();
				doOldStyle(listNoTmp, listCoTmp, listOutTmp, listNoAfter,
						listCoAfter, listSaiTmp);
			}
			listOutTmp.clear();
			listCoTmp.clear();
			listSaiTmp.clear();
	...
	private void doOldStyle(List<InputData> listNoKey,
			List<InputData> listCoKey, List<OutputData> listOutTmp,
			List<InputData> listNoAfter, List<InputData> listCoAfter,
			List<InputClone> listSaiTmp) {
		double sum2 = Math.round(tinhTongSoTienIn(listCoKey) * 100) / 100.0;
		// bangNhau = true;
		// if (bangNhau) {
		// Tìm các cặp bằng nhau
		for (int i = 0; i < listNoKey.size(); i++) {
		...
		for (int i = 0; i < listNoAfter.size(); i++) {
			InputData conNo = listNoAfter.get(i);
			// Đoạn code để lấy các giá trị = nhau trước
			InputData equalCo = Common.getEqualData(conNo, listCoAfter);
			if (equalCo.getTaiKhoan() != null) {
				OutputData outputData = setOutputData(conNo, equalCo);
		...
		while (listNoAfter.size() > 0 && listCoAfter.size() > 0) {
			...
			if (firstNo.getPsNo() > firstCo.getPsCo()) {
				...
				for (InputData inputData : tongHopCacSoHang) {
					OutputData outputData = setOutputData(inputData, firstNo);
					listOutTmp.add(outputData);
					listCoAfter.remove(inputData);
				}
				...
				for (InputData inputData : tongHopCacSoHang) {
					OutputData outputData = setOutputData(inputData, firstCo);
					listOutTmp.add(outputData);
					listNoAfter.remove(inputData);
				}
	...
	private void copySameData(List<InputData> listSaiTmp, List<InputData> listKey) {
		for (int i = 0; i < listKey.size(); i++) {
			listSaiTmp.add(new InputData(listKey.get(i)));
		}
	...
	private void diTimSoHang(List<InputData> listInput, double target,
		...
		if (cacListThoaMan.size() == 1 || currentTime - startTime >= 100) {
			return;
		}
