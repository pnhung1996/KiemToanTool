...
public class MakeMeCry {
	...
	public void ghiRaFile(String filePath) {
	...
	public void ghiRafile(String filePath1) {
		try {
			...
			for (OutputData outputData : listDataOut) {
				...
				for (int i = 0; i < numberColumn; i++) {
					writer.write(",");
					writer.write(outputData.getColumnNumber(2 * i).replace(',', 'ⱴ'));
					writer.write(",");
					writer.write(outputData.getColumnNumber(2 * i + 1).replace(',', 'ⱴ'));
				}
				writer.write("\r\n");
			}
			writer.flush();
